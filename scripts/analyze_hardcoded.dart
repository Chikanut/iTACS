...
class HardcodedAnalyzer {
  ...
  final textPatterns = [
    ...
    TextPattern(r'"[А-Яа-яІіЇїЄєҐґ][^"]*"', 'Українські рядки в подвійних лапках'),
    TextPattern(r"'[A-Za-z][A-Za-z\s]{3,}[^']*'", 'Англійські рядки (>3 символи)'),
    TextPattern(r'"[A-Za-z][A-Za-z\s]{3,}[^"]*"', 'Англійські рядки в подвійних лапках'),
  ...
  void _generateColorReport(StringBuffer buffer) {
    ...
    colorsByFile.forEach((file, issues) {
      ...
      for (final issue in issues.take(5)) { // Показуємо перші 5
  ...
  void _generateTextReport(StringBuffer buffer) {
    ...
    for (final text in sortedTexts.take(20)) { // Показуємо перші 20
      buffer.writeln('   • "$text"');
    }
    ...
    for (final entry in sortedFiles.take(10)) {
      ...
      for (final issue in entry.value.take(3)) {
        buffer.writeln('   Рядок ${issue.lineNumber}: "${issue.extractedText}"');
      }
  ...
  void _generateSummary(StringBuffer buffer) {
    ...
    buffer.writeln('• Унікальних текстів: ${textIssues.map((e) => e.extractedText).toSet().length}');
    buffer.writeln('• Файлів з кольорами: ${colorIssues.map((e) => e.file).toSet().length}');
    buffer.writeln('• Файлів з текстами: ${textIssues.map((e) => e.file).toSet().length}');
  ...
  void _generateRecommendations(StringBuffer buffer) {
    ...
    if (colorIssues.isNotEmpty) {
      ...
      buffer.writeln('   2. Замініть Colors.* на Theme.of(context).colorScheme.*');
    ...
    if (textIssues.isNotEmpty) {
      ...
      buffer.writeln('   3. Замініть hardcoded тексти на AppLocalizations.of(context).key');
