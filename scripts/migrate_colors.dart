...
class SmartColorMigrator {
  ...
  FileContext _analyzeFileContext(String content, String filePath) {
    return FileContext(
      hasContext:
          content.contains('BuildContext') || content.contains('Widget'),
      hasThemeImport: content.contains(
        "import 'package:flutter/material.dart'",
      ),
      isServiceFile: filePath.contains('/services/'),
      isUtilFile:
          filePath.contains('/utils/') || filePath.contains('_utils.dart'),
      isModelFile:
          filePath.contains('/models/') || filePath.contains('_model.dart'),
      isWidgetFile:
          filePath.contains('/widgets/') || filePath.contains('/pages/'),
  ...
  String _fixExistingIssues(String content, FileContext context) {
    ...
    final constThemeRegex = RegExp(
      r'const\s+(\w+)\((.*?)Theme\.of\(context\)(.*?)\)',
    );
    content = content.replaceAllMapped(constThemeRegex, (match) {
      fixes++;
      replacementTypes['Прибрано const'] =
          (replacementTypes['Прибрано const'] ?? 0) + 1;
    ...
    content = content.replaceAllMapped(
      RegExp(
        r'Theme\.of\(context\)\.colorScheme\.(primary|secondary|tertiary|error)\.shade(\d+)',
      ),
      (match) {
        ...
        replacementTypes['ColorScheme.shade → MaterialColor.shade'] =
            (replacementTypes['ColorScheme.shade → MaterialColor.shade'] ?? 0) +
            1;
    ...
    if (context.isServiceFile) {
      content = content.replaceAllMapped(
        RegExp(r'Theme\.of\(context\)\.colorScheme\.(\w+)'),
        (match) {
          final colorProperty = match.group(1);
          final staticColor = _getStaticColorForProperty(colorProperty!);
          fixes++;
          replacementTypes['Service → статичний колір'] =
              (replacementTypes['Service → статичний колір'] ?? 0) + 1;
          return staticColor;
        },
      );
    }
    ...
    if (content.contains('Theme.of(context)') && !context.hasContext) {
      manualFixesNeeded.add(
        '${context.filePath}: Потрібен context або статичні кольори',
      );
    }
  ...
  String _fixShadeProperties(String content) {
    ...
    content = content.replaceAllMapped(
      ...
      (match) {
        ...
        replacementTypes['Error shade → dangerRed'] =
            (replacementTypes['Error shade → dangerRed'] ?? 0) + 1;
    ...
    content = content.replaceAllMapped(
      ...
      (match) {
        final shade = match.group(1);
        replacementTypes['Primary shade → primaryBlue'] =
            (replacementTypes['Primary shade → primaryBlue'] ?? 0) + 1;
    ...
    content = content.replaceAllMapped(
      ...
      (match) {
        final shade = match.group(1);
        replacementTypes['Secondary shade → warningOrange'] =
            (replacementTypes['Secondary shade → warningOrange'] ?? 0) + 1;
    ...
    content = content.replaceAllMapped(
      RegExp(
        r'AppTheme\.(primaryBlue|secondaryGreen|warningOrange|dangerRed|greyScale)\.shade(\d+)',
      ),
      (match) {
    ...
    legacyShadeToOpacity.forEach((shade, opacity) {
      ...
      final pattern = RegExp(
        r'(Theme\.of\(context\)\.colorScheme\.(?:onSurface|onSurfaceVariant|outline))\.$shade',
      );
      content = content.replaceAllMapped(pattern, (match) {
        replacementTypes['Legacy shade → opacity'] =
            (replacementTypes['Legacy shade → opacity'] ?? 0) + 1;
  ...
  String _migrateWithStaticColors(String content, FileContext context) {
    ...
    for (final mapping in colorMappings) {
      ...
      if (regex.hasMatch(content)) {
        ...
        replacementTypes['Static colors with shade'] =
            (replacementTypes['Static colors with shade'] ?? 0) + 1;
      }
    }
    ...
    for (final mapping in colorMappings) {
      ...
      content = content.replaceAllMapped(regex, (match) {
        ...
        final constMatch = RegExp(
          r'const\s+\w+\([^)]*$',
        ).hasMatch(beforeMatch.split('\n').last);

        if (constMatch) {
          // Залишаємо як є і додаємо до списку ручних виправлень
          manualFixesNeeded.add(
            '${context.filePath}:${_getLineNumber(content, matchStart)} - const конфлікт',
          );
          return match.group(0)!;
        }

        replacementTypes['Theme colors with shade'] =
            (replacementTypes['Theme colors with shade'] ?? 0) + 1;
        return mapping[1];
      });
    }
  ...
  void printReport() {
    ...
    print(
      '   2. Якщо є помилки: dart scripts/smart_migrate_colors.dart --fix-existing',
    );
