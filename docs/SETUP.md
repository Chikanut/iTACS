...
- Для роботи `Materials` і `Tools` створіть документ `drive_catalog_by_group/{groupId}` з полями `materialsFolderId` і `toolsRootFolderId`.
- Папки в Google Drive мають бути розшарені саме на тих користувачів або групи, які повинні бачити відповідні файли в застосунку.
- Клієнт напряму звертається до Google Drive API від імені користувача, тому недоступні в Drive файли не з'являються в списках навіть за наявності Firestore overlay.
- Upload і створення папок використовують scope `drive.file`, тому такі сценарії варто давати лише тим ролям, які реально редагують каталог.
