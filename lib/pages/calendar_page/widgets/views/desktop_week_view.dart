...
class DesktopWeekView extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Column(
      children: [
        ...
        Expanded(child: _buildTimeView(context)),
      ],
    );
  }
  ...
  Widget _buildDateHeader(BuildContext context) {
    ...
    return SizedBox(
      ...
      child: Row(
        children: [
          SizedBox(width: timeColumnWidth, child: Container()),
    ...
    return Container(
      ...
      child: Stack(
        children: [
          ...
          // Заняття
          ...dayLessons.map(
            (lesson) => _buildPositionedLesson(lesson, dayIndex),
          ),
        ],
      ),
    );
  ...
  Widget _buildPositionedLesson(LessonModel lesson, int dayIndex) {
    ...
    final topPosition =
        (start.hour - minHour) * hourHeight + (start.minute * minuteHeight);
    final height = (durationMinutes * minuteHeight).clamp(
      50.0,
      double.infinity,
    );
    ...
    final overlappingLessons = allDayLessons
        .where(
          (other) => CalendarUtils.timesOverlap(
    ...
    overlappingLessons.sort((a, b) {
      ...
      return a.endTime.millisecondsSinceEpoch.compareTo(
        b.endTime.millisecondsSinceEpoch,
      );
    });
  ...
  Widget _buildDesktopLessonCard(LessonModel lesson) {
    ...
    final readinessStatus = LessonStatusUtils.getReadinessStatus(
      lesson,
    ); // ⬅️ нове
