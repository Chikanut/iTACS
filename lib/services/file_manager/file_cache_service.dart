...
class FileCacheService {
  ...
  Future<void> cacheFile({
    ...
    try {
      ...
      debugPrint(
        'FileCacheService: Збережено файл: $fileId (${entry.humanReadableSize})',
      );
    } catch (e) {
  ...
  Future<void> updateCachedFile({
    ...
    try {
      ...
      debugPrint(
        'FileCacheService: Оновлено файл: $fileId (${updatedEntry.humanReadableSize})',
      );
    } catch (e) {
  ...
  Future<int> cleanOldFiles(Duration maxAge) async {
    ...
    debugPrint(
      'FileCacheService: Видалено ${filesToRemove.length} старих файлів',
    );
  ...
  Future<int> cleanFilesByType(String extension) async {
    ...
    debugPrint(
      'FileCacheService: Видалено ${filesToRemove.length} файлів типу $extension',
    );
    return filesToRemove.length;
  }
  ...
  Map<String, dynamic> getCacheStatistics() {
    ...
    for (final entry in files) {
      extensionStats[entry.extension] =
          (extensionStats[entry.extension] ?? 0) + 1;
      if (entry.mimeType != null) {
        mimeTypeStats[entry.mimeType!] =
            (mimeTypeStats[entry.mimeType!] ?? 0) + 1;
      }
    }
    ...
    return {
      ...
      'oldestFile': files.isEmpty
          ? null
          : files.reduce(
              (a, b) =>
                  (a.modifiedDateTime?.isBefore(
      ...
      'newestFile': files.isEmpty
          ? null
          : files.reduce(
              (a, b) =>
                  (a.modifiedDateTime?.isAfter(
                        b.modifiedDateTime ?? DateTime.now(),
                      ) ??
                      false)
                  ? a
      ...
      'largestFile': files.isEmpty
          ? null
          : files.reduce((a, b) => (a.size ?? 0) > (b.size ?? 0) ? a : b),
    };
  }
  ...
  Future<List<String>> validateCacheIntegrity() async {
    ...
    debugPrint(
      'FileCacheService: Знайдено і видалено ${corruptedFiles.length} пошкоджених файлів',
    );
