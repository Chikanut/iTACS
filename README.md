...
У проєкті використовується трирівнева модель доступу: `viewer`, `editor`, `admin`.

### Viewer

- бачить календар, dashboard, матеріали, інструменти, статистику та профілі;
- може редагувати власний профіль і налаштування сповіщень;
- може завантажувати й відкривати доступні файли;
- може подавати власні заявки на відсутність і скасовувати власні `pending` заявки;
- може генерувати доступні звіти;
- якщо призначений викладачем заняття, може:
  - підтвердити ознайомлення із заняттям;
  - заповнювати `customFieldValues` для цього заняття;
- не може створювати, редагувати, видаляти заняття;
- не може самостійно призначати або знімати себе із заняття;
- не може керувати матеріалами, тулзами, шаблонами групи чи учасниками.

### Editor

- має всі права `viewer`;
- може створювати, редагувати й видаляти заняття;
- може призначати та змінювати викладачів для занять;
- може додавати, редагувати й видаляти матеріали та інструменти;
- може керувати груповими шаблонами, автодоповненням локацій, підрозділів і тегів;
- не має доступу до адмін-панелі;
- не керує ролями учасників, системними notifications, конфігом Drive-каталогу чи шаблонами звітів.

### Admin

- має всі права `editor`;
- має повний доступ до адмін-панелі;
- керує складом групи та ролями користувачів;
- підтверджує, відхиляє, призначає та скасовує відсутності;
- керує шаблонами звітів, груповими notifications і `drive_catalog_by_group`;
- виконує адміністративні дії, що впливають на конфіг групи та доступ інших користувачів.

### Окремі правила для занять

- повторне ознайомлення потрібне лише після зміни дати заняття або `unit`;
- ознайомлення із заняттям підтверджує саме призначений викладач, а не роль `editor/admin` сама по собі;
- право редагувати `customFieldValues` для конкретного заняття має призначений викладач або користувач із роллю `editor/admin`.
